...
pub async fn run(opts: &RestoreOpts) -> Result<()> {
    ...
    let (_multi_progress, pg_bar, decomp_bar, decrypt_bar, s3_bar) = if use_simple {
        ...
        // Create progress bars for each stage (in pipeline order)
        let s3_bar = Arc::new(multi_progress.add(ProgressBar::new(0)));
        let decrypt_bar = Arc::new(multi_progress.add(ProgressBar::new(0)));
        let decomp_bar = Arc::new(multi_progress.add(ProgressBar::new(0)));
        let pg_bar = Arc::new(multi_progress.add(ProgressBar::new(0)));

        // Set styles and initial messages
        for bar in [&pg_bar, &decomp_bar, &decrypt_bar, &s3_bar] {
        ...
        decomp_bar.enable_steady_tick(refresh_rate);
        decrypt_bar.enable_steady_tick(refresh_rate);
        s3_bar.enable_steady_tick(refresh_rate);

        // Set initial messages
        s3_bar.set_message("Downloading from S3...");
        decrypt_bar.set_message("Decrypting...");
        decomp_bar.set_message("Decompressing...");
        pg_bar.set_message("Restoring PostgreSQL...");

        (Some(multi_progress), pg_bar, decomp_bar, decrypt_bar, s3_bar)
    };
